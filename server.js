...
router.route('/movies')
    ...
    .post(authJwtController.isAuthenticated, async (req, res) => {
        if (!req.body.title || !req.body.actors || req.body.actors.length === 0) {
            return res.status(400).json({ success: false, message: 'Title and at least one actor are required.' });
        }
        try {
    ...
    .put(authJwtController.isAuthenticated, async (req, res) => {
        try {
            ...
            res.json({ success: true, message: 'Movie updated', movie: updatedMovie });
        } catch (error) {
